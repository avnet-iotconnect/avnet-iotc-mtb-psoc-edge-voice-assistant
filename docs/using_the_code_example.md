...
The following example clones the "[DEEPCRAFT&trade; Voice Assistant deployment](https://github.com/Infineon/mtb-example-psoc-edge-voice-assistant-deploy)" application with the desired name "VoiceAssistantDeploy" configured for the *KIT_PSE84_EVAL_EPC2* BSP into the specified working directory, *C:/mtb_projects*:

   ```
   project-creator-cli --board-id KIT_PSE84_EVAL_EPC2 --app-id mtb-example-psoc-edge-voice-assistant-deploy --user-app-name VoiceAssistantDeploy --target-dir "C:/mtb_projects"
   ```
...
<details><summary><b>Using CLI</b></summary>
   ...
   Example:
   ```
   make program TOOLCHAIN=LLVM_ARM
   ```
</details>
